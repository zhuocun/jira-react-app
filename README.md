...
For the **v2 redesign** of the AI features — a single agent over an SSE wire with FE↔BE tool calling, an autonomy dial, an Action Audit + Undo, a triage Inbox, a `Cmd/Ctrl+K` palette, and inline `/copilot` slash commands — see [docs/prd/board-copilot-v2-agent.md](docs/prd/board-copilot-v2-agent.md). The v1 local engine remains the read-only fallback when the agent server is unreachable.
